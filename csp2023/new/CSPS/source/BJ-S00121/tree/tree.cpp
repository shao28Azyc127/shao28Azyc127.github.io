...
int main(){
    ...
    while(pq2.top().first+(pq2.top().first+max(p[pq2.top().second].b+x*p[pq2.top().second].c,1))<p[pq2.top().second].a)pq2.top().first=make_pair(pq2.top().first+max(p[pq2.top().second].b+x*p[pq2.top().second].c,1),pq2.top.second),x++;
    printf("%d",x);
    return 0;
}
